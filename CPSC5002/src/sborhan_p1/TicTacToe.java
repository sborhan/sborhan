...
public class TicTacToe {
    ...
    public boolean winCondition() {
        ...
        for (int i = 0; i < gameBoardGrid.length; i++) {
            for (int j = 0; j < gameBoardGrid.length ; j++) {
                if (gameBoardGrid[i][j].equals("X")) {
                    (loopCounter1)++;
                }
                if (gameBoardGrid[i][j].equals("O") ) {
                    (loopCounter11)++;
                } else if (gameBoardGrid[j][i].equals("X") ) {
                    (loopCounter2)++;
                } else if (gameBoardGrid[j][i].equals("O") ) {
                    (loopCounter22)++;
                } else if (gameBoardGrid[j][i].equals("X") ) {
                    (loopCounter3)++;
                } else if (gameBoardGrid[j][i].equals("O") ) {
        ...
        if (loopCounter1 == gameBoardGrid.length  ||
                loopCounter11 == gameBoardGrid.length  ||
                loopCounter2 == gameBoardGrid.length  ||
                loopCounter22 == gameBoardGrid.length  ||
                loopCounter3 == gameBoardGrid.length ||
                loopCounter33 == gameBoardGrid.length  ||
                loopCounter4 == gameBoardGrid.length  ||
                loopCounter44 == gameBoardGrid.length ) {
            return true;
